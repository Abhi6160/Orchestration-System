...
cp .env.example .env
# edit .env and add keys for whichever providers you actually use in
# app/config.py MODEL_TIERS. Four providers ship out of the box:
#   ANTHROPIC_API_KEY, OPENAI_API_KEY, GROQ_API_KEY, HUGGINGFACE_API_KEY
# Cloudflare Workers AI needs two values, not just one:
#   CLOUDFLARE_API_TOKEN, CLOUDFLARE_ACCOUNT_ID
#
# .env is git-ignored — your real keys never get committed as long as you
# edit .env, not .env.example. Don't paste real keys into .env.example,
...
[Call that tier's provider] (providers/anthropic_provider.py, openai_provider.py,
                              providers/groq_provider.py, huggingface_provider.py,
...
| `app/providers/groq_provider.py` | Groq backend (OpenAI-compatible API, different base URL) |
| `app/providers/huggingface_provider.py` | Hugging Face backend (Inference Providers router) |
| `app/providers/cloudflare_provider.py` | Cloudflare Workers AI backend (needs account ID + token) |
...
**Notes on the Groq / Hugging Face / Cloudflare providers**:
- Groq reuses the `openai` SDK pointed at Groq's base URL (its Chat
  Completions API is OpenAI-compatible) — no extra dependency needed.
- Hugging Face goes through the Inference Providers router
  (`router.huggingface.co`), which only serves models that have an
  Inference Provider attached — check the model card's "Inference
  Providers" section before picking a `model_id`.
- Cloudflare Workers AI needs `CLOUDFLARE_ACCOUNT_ID` in addition to the
  token, and `model_id` must be Cloudflare's full path (e.g.
  `@cf/meta/llama-3.1-8b-instruct-fast`), not a bare model name.
- None of these three has a well-documented, stable error code for
  "context window exceeded" the way Anthropic/OpenAI do, so their overflow
  detection is a best-effort substring match on the error body — check the
  raw response in the logs first if a handoff doesn't trigger when you'd
  expect.
- `model_id`s and prices in `config.py` for these three are placeholders —
  verify against each provider's current docs/pricing before trusting the
  cost numbers in logs.
