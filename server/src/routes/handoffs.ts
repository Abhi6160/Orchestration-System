...
import { ChatMessage, HandoffPayload } from '../providers/types.js';
...
router.post('/:id/handoff/preview', async (req: Request, res: Response) => {
  try {
    ...
    const { to_provider = 'gemini' } = req.body;
    ...
    const preview = HandoffEngine.generateHandoff(
      id,
      chatMessages,
      conversation.current_provider,
      to_provider as 'claude' | 'gemini',
...
router.post('/:id/handoff/confirm', async (req: Request, res: Response) => {
  try {
    ...
    const newContextLimit = to_provider === 'gemini' ? 2000000 : 200000;
    const newTokens = compressed_token_estimate || 1850;

    dbService.updateConversation(id, {
      current_provider: to_provider as 'claude' | 'gemini',
      context_limit: newContextLimit,
      total_tokens: newTokens
    });

    // 3. Inject Transition Handoff Anchor Message
    const transitionContent = `[ContextBridge Handoff Complete: Context transferred from ${from_provider === 'claude' ? 'Claude 3.5 Sonnet' : 'Gemini 1.5 Pro'} to ${to_provider === 'gemini' ? 'Gemini 1.5 Pro' : 'Claude 3.5 Sonnet'}. Reduced ${(original_token_count || conversation.total_tokens).toLocaleString()} tokens to ${(compressed_token_estimate || 1850).toLocaleString()} token structured context digest.]`;
    ...
    if (auto_continue) {
      const adapter = to_provider === 'gemini' ? gemini : claude;
      const handoffPayload: HandoffPayload = {
        conversationId: id,
        fromProvider: from_provider as 'claude' | 'gemini',
        toProvider: to_provider as 'claude' | 'gemini',
        userGoal: user_goal || '',
