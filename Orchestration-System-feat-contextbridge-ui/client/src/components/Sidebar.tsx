  ...
  return (
    <nav className="w-[280px] h-screen fixed left-0 top-0 bg-surface border-r border-outline-variant flex flex-col py-4 z-20 select-none">
      ...
      <div className="px-4 mb-5 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-semibold text-base tracking-tight text-primary leading-tight">ContextBridge</h1>
          <p className="text-xs text-on-surface-variant font-medium">AI Workspace</p>
        </div>
      </div>
        ...
        <div>
          ...
          <ul className="space-y-0.5 mt-1">
            {conversations.map((conv) => {
              ...
              const providerDot = getProviderMeta(conv.current_provider).dot;
                ...
                <li key={conv.id} className="group relative">
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0 flex-1 mr-1">
                      <span
                        className={`w-1.5 h-1.5 rounded-full shrink-0 ${providerDot}`}
                      />
      ...
      <div className="px-3 mt-auto border-t border-outline-variant pt-3 space-y-2">
        ...
        <ul className="space-y-0.5 text-xs text-on-surface-variant">
          <li>
            <a
              href="#help"
              className="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-surface-container transition-colors"
            >
          ...
          <li>
            <div className="flex items-center justify-between px-2 py-1.5">
              <span className="flex items-center gap-2">
                ...
                <span className="text-on-surface-variant">System Ready</span>
              </span>
              <span className="text-[10px] text-on-surface-variant/60">v1.0.0</span>
            </div>
          </li>
        </ul>
      </div>
