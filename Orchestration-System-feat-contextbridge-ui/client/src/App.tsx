...
import { ProviderTransition } from './components/ProviderTransition';
...
import { Folder, Calendar, Loader2 } from 'lucide-react';
...
export const App: React.FC = () => {
  ...
  const handleTriggerHandoff = async (targetProvider?: ProviderId) => {
    if (!activeConversationId) return;
    try {
      setIsHandoffModalOpen(true);
      setIsLoadingPreview(true);
      const preview = await api.generateHandoffPreview(activeConversationId, targetProvider);
  ...
  const showWarning = (activeDemoState === 'context_warning' || usagePercent >= 80) && !isWarningDismissed;
    ...
    <div className="flex h-screen w-full bg-background text-on-background font-sans antialiased overflow-hidden selection:bg-secondary-fixed selection:text-on-secondary-fixed">
      ...
      <div className="ml-[280px] w-[calc(100%-280px)] h-screen flex flex-col relative">
              ...
              <>
                ...
                {messages.map((msg) => {
                  if (msg.role === 'system' || msg.provider === 'system') {
                    // State 5 Anchor: Provider Transition Divider
                    return <ProviderTransition key={msg.id} message={msg} />;
                  }
                  return (
                    <MessageBubble
                      key={msg.id}
                ...
                {isSendingMessage && (() => {
                  const pendingMeta = getProviderMeta(currentProvider);
                  const PendingIcon = pendingMeta.icon;
                  return (
                    <div className="flex items-start gap-2.5 max-w-[90%] opacity-80 animate-pulse">
                      <div
                        className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${pendingMeta.iconBg} ${pendingMeta.iconText}`}
                      >
                      ...
                      <div className="bg-surface-container-lowest px-4 py-3 rounded-xl rounded-tl-xs border border-outline-variant shadow-2xs flex items-center gap-2">
                        <div
                          className={`w-2 h-2 rounded-full animate-bounce ${pendingMeta.barColor}`}
        ...
        <HandoffModal
          ...
          onChangeTarget={(provider) => handleTriggerHandoff(provider)}
        />
