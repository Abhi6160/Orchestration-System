...
export const Composer: React.FC<ComposerProps> = ({
  ...
  const limit = contextLimit || (isClaude ? 200000 : 2000000);
  const percent = Math.min(100, Math.round((totalTokens / limit) * 100));
  const isHighContext = isClaude && percent >= 80;
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-background via-background/95 to-transparent pt-6 pb-4 px-6 flex justify-center z-10 select-none">
      <div className="w-full max-w-[800px] flex flex-col gap-2">
        ...
        {/* Input Box */}
        <div
          className={`relative bg-surface-container-lowest border rounded-xl shadow-xs transition-all ${
            isClaude
              ? 'border-outline-variant focus-within:border-secondary focus-within:ring-2 focus-within:ring-secondary/20'
              : 'border-outline-variant focus-within:border-blue-600 focus-within:ring-2 focus-within:ring-blue-600/20'
          }`}
        >
          <textarea
            ref={textareaRef}
            ...
            disabled={isLoading}
            placeholder={isClaude ? 'Message Claude 3.5 Sonnet...' : 'Message Gemini 1.5 Pro...'}
            rows={1}
