...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <div className="w-full max-w-[800px] py-8 flex flex-col items-center text-center animate-in fade-in duration-300">
      ...
      <div className="flex items-center gap-2 p-1 bg-surface-container rounded-xl border border-outline-variant/60 mb-8 shadow-2xs">
        <button
          onClick={() => onChangeProvider('claude')}
          className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
            isClaude
              ? 'bg-surface-container-lowest text-secondary shadow-xs border border-secondary-fixed-dim/40'
              : 'text-on-surface-variant hover:text-primary'
          }`}
        >
          <BrainCircuit className="w-3.5 h-3.5" />
          <span>Claude 3.5 Sonnet</span>
          <span className="text-[10px] opacity-75 font-mono">200k</span>
        </button>
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
            !isClaude
              ? 'bg-surface-container-lowest text-blue-700 shadow-xs border border-blue-200'
              : 'text-on-surface-variant hover:text-primary'
          }`}
        >
          <Sparkles className="w-3.5 h-3.5" />
          <span>Gemini 1.5 Pro</span>
          <span className="text-[10px] opacity-75 font-mono">2M</span>
        </button>
      </div>
