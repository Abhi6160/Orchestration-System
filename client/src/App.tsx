...
export const App: React.FC = () => {
  ...
  const [handoffPreview, setHandoffPreview] = useState<HandoffPreview | null>(
    null,
  );
  const [isLoadingPreview, setIsLoadingPreview] = useState(false);
  const [activeDemoState, setActiveDemoState] =
    useState<DemoStateId>("active_claude");
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      if (
        result.conversation.total_tokens >= 160000 &&
        result.conversation.current_provider === "claude"
      ) {
        setActiveDemoState("context_warning");
      }
    } catch (err) {
  ...
  const handleTriggerHandoff = async () => {
    ...
    try {
      ...
      const preview = await api.generateHandoffPreview(
        activeConversationId,
        "gemini",
      );
  ...
  const handleDemoStateSelect = async (stateId: DemoStateId) => {
    ...
    if (stateId === "empty") {
      ...
      if (
        currentConversation?.current_provider !== "gemini" ||
        !messages.some((m) => m.provider === "system")
      ) {
        await api
          .generateHandoffPreview("apollo-launch-planning", "gemini")
          .then(async (preview) => {
  ...
  const showWarning =
    (activeDemoState === "context_warning" ||
      (usagePercent >= 80 && currentProvider === "claude")) &&
    !isWarningDismissed;
                ...
                onChangeProvider={(p) => {
                  if (activeConversationId) {
                    ...
                    setCurrentConversation((prev) =>
                      prev ? { ...prev, current_provider: p } : null,
                    );
                  }
                }}
                ...
                <div className="border-b border-outline-variant pb-3 mb-1">
                  ...
                  <div className="flex gap-2 mt-2">
                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-surface-container text-on-surface-variant text-[11px] font-medium border border-outline-variant/40">
                      ...
                      <span>
                        Project: {currentConversation?.project_tag || "Apollo"}
                      </span>
                    </span>
                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-surface-container text-on-surface-variant text-[11px] font-medium border border-outline-variant/40">
